...
/**	Kims funktioner - I/O funktioner kompatibel med GPIO: */

STATIC INLINE void DigitalOut(uint8_t port, uint8_t pin)
{
	Chip_GPIO_SetPinDIROutput(LPC_GPIO, port, pin);
}

STATIC INLINE void DigitalIn(uint8_t port, uint8_t pin)
{
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, port, pin);
}

STATIC INLINE void DigitalWrite(uint8_t port, uint8_t pin, bool setting)
{
	Chip_GPIO_SetPinState(LPC_GPIO, port, pin, setting);
}

STATIC INLINE bool DigitalRead(uint8_t port, uint8_t pin)
{
